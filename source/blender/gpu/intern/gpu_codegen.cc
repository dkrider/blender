...
static std::ostream &operator<<(std::ostream &stream, GPUComparisonType cmp)
{
  switch (cmp) {
    case GPU_CMP_NE:
      return stream << "!=";
...
class GPUCodegenSerializer {
 ...
 private:
  ...
  bool is_blank_conditional(const GPUNodeConditional *cond);
  bool is_blank_conditional(const GPUNode *node);
 ...
 public:
  GPUCodegenSerializer(GPUNodeGraph &graph, eGPUNodeTag tree_tag)
      : graph(graph),
        tree_tag(tree_tag),
  ...
  void serialize(std::stringstream &eval_ss, GPUNodeLink *output_link = nullptr);
};
...
bool GPUCodegenSerializer::remap_output_to_input(const GPUInput *to, const GPUOutput *from)
{
  ...
  if (to->source == GPU_SOURCE_OUTPUT && to->link->output->type == from->type) {
    ...
    if (cur_id == from->id) {
      output_map[from->id] = find_remapped_output(to->link->output->id);
      return true;
    }
  }
...
bool GPUCodegenSerializer::is_blank_conditional(const GPUNodeConditional *cond)
{
  return cond->branch_false < 0 && branch_info[cond->branch_true].nodes.empty();
}

bool GPUCodegenSerializer::is_blank_conditional(const GPUNode *node)
{
  return node->type == GPU_NODE_CONDITIONAL &&
         is_blank_conditional(reinterpret_cast<const GPUNodeConditional *>(node));
}
                                               ...
                                               const GPUNodeConditional *cond)
{
  ...
  if (!is_blank_conditional(cond)) {
    ...
    else if (output->type == GPU_CLOSURE) {
      eval_ss << indent << "else {\n" << indent_nested;
      node_serialize_output(eval_ss, output);
      eval_ss << " = CLOSURE_DEFAULT;\n" << indent << "}\n";
    }
...
void GPUCodegenSerializer::serialize(std::stringstream &eval_ss, GPUNodeLink *output_link)
{
  build_branches();
...
char *GPUCodegen::graph_serialize(eGPUNodeTag tree_tag, GPUNodeLink *output_link)
{
  ...
  serializer.serialize(eval_ss, output_link);
